...
def plot_boxplot(ax, data, title, colors):
    all_data = []
    for agent in data:
    ...
    for idx, model_data in enumerate(all_data):
        ...
        ax.boxplot(model_data, positions=[positions[idx]], patch_artist=True,
                   ...
                   medianprops=dict(color=boxprops["color"]))
    
    ax.set_title(title, fontsize=14)
    ax.set_xticks([1.5, 3.5])  # Position the ticks between the grouped models
    ax.set_xticklabels(['CoTAgents', 'GenerativeAgents'], fontsize=12)
    ax.grid(True)
...
# Plot each indicator
plot_boxplot(axs[0], defect_percentages_data, "Defect Percentage Across Scenarios", subcat_colors)
plot_boxplot(axs[1], data_second, "Proportion of Red Coins Taken Over Yellow Coins", subcat_colors)
plot_boxplot(axs[2], data_third, "Proportion of Explore or Stay Actions When Red Coin Visible", subcat_colors)
...
fig.suptitle("Coins Scenario 1 - Agents Performance Indicators", fontsize=16)
axs[0].set_ylabel("Performance")

# Adjust layout and save the figure
plt.tight_layout()
plt.savefig("boxplot_comparison.png", dpi=600)
plt.show()