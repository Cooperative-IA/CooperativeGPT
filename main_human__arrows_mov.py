...
def game_loop(agents: list[Agent | HumanAgentArrowsMov], substrate_name:str, persist_memories:bool, communication_handler: CommunicationHandler) -> None:
    ...
    # We are goint to wait for the agents to start the game, they will send a message to the communication handler
    # when they are ready to start the game
    communication_handler.wait_for_agents_to_start()
    ...
    while rounds_count < max_rounds and not condition_to_end_game(substrate_name, env.get_current_global_map()):
        ...
        for agent in agents:
            ...
            communication_handler.publish_data_topic(player_images, agents_orientations, id_turn, "Aloooo observaciones mi papa")
            ...
            if check_agent_out_of_game(observations):
                ...
                agent.move(observations, scene_description, state_changes, env.get_current_global_map(), game_time, communication_handler, agent_reward , agent_is_out=True)
            else:
                agent.move(observations, scene_description, state_changes, env.get_current_global_map(), game_time, communication_handler, agent_reward)




                
        # Start time counter for this round
        ...
        while time.time() - round_start_time < 0.1:
            try:
                ...
                continue
        print("--------------------------------")
        print("Actions received after 0.2 seconds")
        print(received_actions)
        print("--------------------------------")
        # Keep only the last action for each agent_id
        agent_actions = {}
        for agent_id, action in received_actions:
            agent_actions[agent_id] = action
            # Print the action received
            print(f"Action received for agent {agent_id}: {action}")
        
        # Execute the move for each agent
        for agent in agents:
            observations = all_observations['curr_state']
            
            # TODO: USAR SOLO EL STEP ACTION,  que los actions recibidos sean los steps puntuales
            # Luego le hago update al agente para su memoria o en donde haga falta.
            
            if check_agent_out_of_game(observations):
                logger.info(f'Agent {agent.name} was taken out of the game. Skipping agent move.')
                step_action = None
            else:
                agent_action = agent_actions.get(agent.agent_id, "stay put")
                print(f"Agent {agent.name} action: {agent_action}")
                #step_action = agent.generate_new_actions(agent_action)
                step_action = agent_action
        ...
        try:
            env.step(actions)
            env.game_recorder.record_game_actions(actions)
            steps_count += 1
        except:
...
if __name__ == "__main__":
    ...
    try:
        game_loop(agents, args.substrate, args.persist_memories, communication_handler)
