...
def game_loop(agents: list[Agent | HumanAgent], substrate_name:str, persist_memories:bool, communication_handler: CommunicationHandler) -> None:
    ...
    while rounds_count < max_rounds and not condition_to_end_game(substrate_name, env.get_current_global_map()):
        ...
        for agent in agents:
            ...
            if check_agent_out_of_game(observations):
                ...
                step_action = agent.move(observations, scene_description, state_changes, env.get_current_global_map(), game_time, communication_handler, agent_reward , agent_is_out=True)
            else:
                step_action = agent.move(observations, scene_description, state_changes, env.get_current_global_map(), game_time, communication_handler, agent_reward)
...
if __name__ == "__main__":
    ...
    communication_handler = CommunicationHandler(player_images, players_names, args.substrate)
