...
function AdversarialEvent:__init__(kwargs)
    kwargs = args.parse(kwargs, {
      ...
      {'eventTimeFile', args.stringType},
      {'botsNum', args.ge(0), args.le(10)}
    })
    AdversarialEvent.Base.__init__(self, kwargs)
    self.botsNum = kwargs.botsNum
    self.eventTimeFile = kwargs.eventTimeFile
    self.currentTime = 0
end
...
function AdversarialEvent:update()
    ...
    -- now we read a file with the event times
    -- if the current time is in the file, we execute the event
    local path = self.eventTimeFile
    local file = io.open(path, "r") -- Abre el archivo original en modo de lectura
    if not file then
        local lua_output = io.open("lua_output.txt", "w") -- Abre el archivo destino en modo de escritura
        if not lua_output then
            return
        end        
        lua_output:write("No se pudo abrir el archivo de lectura") -- Escribe "True" en el archivo destino
        lua_output:close() -- Cierra el archivo de escritura
        return
    end

    local adversarialState = file:read("*l") -- Lee la primera línea del archivo
    file:close() -- Cierra el archivo de lectura

    self:executeEvent(adversarialState)


end

function AdversarialEvent:executeEvent(adversarialState)
    local sim = self.gameObject.simulation
    local avatars = sim:getGameObjectsByName("avatar")
    
    for index, avatar in ipairs(avatars) do
        local stateManager = avatar:getComponent("StateManager")
        local avatarsito = avatar:getComponent("Avatar")
        local agentState = stateManager:getState()
        -- aliveState is a string "player"+index
        local aliveState = "player"..index
        local deadState = "playerWait"
        -- adversarialState is not "0"

        if (index >= 4 ) and adversarialState == "0" then
            avatar:setState(deadState)

        elseif (index >= 4 ) and adversarialState == "1" then 
            avatar:setState(aliveState)
        
        elseif (index >= 4 ) and adversarialState == "3" then 
            avatar:setState(deadState)
        end
