...
function AdversarialEvent:update()
    ...
    local primeraLinea = file:read("*l") -- Lee la primera línea del archivo
    file:close() -- Cierra el archivo de lectura

    if primeraLinea == "1" then
        self:executeEvent()
        local file = io.open(path, "w") -- Abre el archivo destino en modo de escritura
        if not file then
            return
        end

        file:write("0") -- Escribe "True" en el archivo destino
        file:close() -- Cierra el archivo de escritura
    end
