...
def communicate_observed_actions_to_agent(sender_agent_name:str, receiver_agent_name:str, agent_registry, observations:list[str], state_changes:list[str], game_time, rounds_count, poignancy, summarized_observations = []) -> None:
    receiver_agent = agent_registry.get_agents([receiver_agent_name])[receiver_agent_name]

    for observation in observations:
        if re.search(r'you were attacked by agent ', observation):
            agent_name_match = re.search(r"attacked by agent (\S+)", observation).group(1)
    ...
    if len(summarized_observations) > 0:
        observations = f'Agent {sender_agent_name} told me that it '
        for i, observation in enumerate(summarized_observations):
            if i == 0:
                observations += observation.replace('Observed', 'observed')
            elif i == len(summarized_observations) - 1:
                observations += ' And finally, it ' + observation.replace('Observed', 'observed')
            else:
                observations += ' Also, it ' + observation.replace('Observed', 'observed')
        receiver_agent.stm.add_known_agent_interaction(rounds_count, sender_agent_name, "observations", observations)
        receiver_agent.ltm.add_memory(f'At {game_time} ' + observations.replace('Agent', 'agent', 1), game_time, poignancy, {'type': 'observation'})
        ...
        if re.search('(?<!I )took an apple from position', state_change):
            ...
            if receiver_agent_name != agent_name_match:
                ...
                if agent_registry.get_agent_turn(sender_agent_name) < agent_registry.get_agent_turn(receiver_agent_name):
                    _rounds_count -= 1
                if receiver_agent.stm.add_known_agent_interaction(_rounds_count, agent_name_match, "ate_apple", f'[{apple_position[0]}, {apple_position[1]}]'):
            ...
            elif re.search(r'(\w+) was attacked', state_change):
                agent_name_match = re.search(r'(\w+) was attacked', state_change).group(1)
                if receiver_agent.stm.add_known_agent_interaction(rounds_count, sender_agent_name, "attacks_made", agent_name_match):
                    ...
                    receiver_agent.ltm.add_memory(f'Agent {sender_agent_name} told me that someone attacked {agent_name_match} at round {rounds_count}', game_time, poignancy, {'type': 'observation'})
...
def communicate_own_actions_to_agent(sender_agent_name:str, receiver_agent_name:str, agent_registry, actions:list[str], rounds_count, game_time, poignancy) -> None:
    ...
    for action in actions:
        if re.search('I took an apple from position', action):
                apple_position = re.search(r'\[(\d+),\s*(\d+)\]', action).groups()
                if receiver_agent.stm.add_known_agent_interaction(rounds_count, sender_agent_name, "ate_apple", f'[{apple_position[0]}, {apple_position[1]}]'):
                    receiver_agent.ltm.add_memory(f'Agent {sender_agent_name} told me that it ate an apple at round {rounds_count}', game_time, poignancy, {'type': 'observation'})
        elif re.search(r'I successfully attacked', action):
            agent_name_match = re.search(r"I successfully attacked (\S+)", action).group(1)
            if receiver_agent.stm.add_known_agent_interaction(rounds_count, sender_agent_name, "attacks_made", agent_name_match):
                receiver_agent.stm.add_known_agent_interaction(rounds_count, agent_name_match, "attacks_received", sender_agent_name)
                receiver_agent.ltm.add_memory(f'Agent {sender_agent_name} told me that it attacked {agent_name_match} at round {rounds_count}', game_time, poignancy, {'type': 'observation'})
...
def communicate_reflection_to_agent(sender_agent_name:str, receiver_agent_name:str, agent_registry, reflection:str, game_time:str, rounds_count:str, poignancy:int) -> None:
    receiver_agent = agent_registry.get_agents([receiver_agent_name])[receiver_agent_name]
    receiver_agent.ltm.add_memory(f'Reflection communicated by {sender_agent_name} at round {rounds_count}:{reflection}', game_time, poignancy, {'type': 'communicated_reflection'})
